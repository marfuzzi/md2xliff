...
function extract(markdownStr, markdownFileName, skeletonFilename, srcLang, trgLang, options) {
    ...
    parseSentencesProcessor()
       .use(ASTtoXliffTransformer)
       .use(xliffCompiler)
       .processSync(markdownStr);

    // Компилирует AST в строку.
    function skeletonCompiler() {
    ...
    function xliffCompiler() {
        ...
        function Compiler(node, file) {
            ...
            //     }
            // });
            units = node.value.map(encodedMarkdown => ({
                id: segmentCounter++,
                source: {
                    lang: srcLang,
            ...
            // console.log(JSON.stringify(units, null , 4));
        }

        function compile() {
            return 'XLIFF COMPILER DATA STUB';
        }
    };

    function ASTtoXliffTransformer() {
        /**
         * @param {MDAST} node узел с markdown строкой.
         * @param {String} text - исходная строка.
         */
        return function(node, text) {
            return {
                type: "xliffEncoded",
                value: node.value.map(str => {
                    var xliffEncodedMarkdown = markdownProcessor()
                        .use(encodeXliffCompiler)
                        .processSync(str);

                    return xliffEncodedMarkdown.contents;
                })
            };
            ...
            function encodeXliffCompiler() {
                this.Compiler = Compiler;
                Compiler.prototype.compile = compile;
                let result = '';

                function Compiler(tree, file) {
                    result += encodeMarkdown(tree);
                }
                ...
                function encodeMarkdown(node) {
                    const { type } = node;
                    if (node.type === 'text') return node.value;

                    function traverseChildren(node, idx) {
                        return node.children.map(child => encodeMarkdown(child, idx)).join('');
                    }
                    ...
                    // Итерируем id для тегов.
                    i++;

                    // TODO: reflink
                    switch(type) {
                        // Для текста не надо итерировать. (?)
                        case 'text':
                            return node.value;
                        case 'link':
                            // Копируем id, т. к. в рекурсивной функции модифицируем i.
                            let j = i;

                            var encoded = [
                                getTag('bpt', `l${j}`, '['),
                                traverseChildren(node), // TODO: убрать грязнохак(ссылка итерирует i на 2)
                                getTag('ept', `l${j}`, ']'),
                                getTag('bpt', `l${j + 1}}`, '('),
                                node.url,
                                getTag('ept', `l${j + 1}`, ')')
                            ...
                            // Т. к. у ссылки 2 пары скобок – увеличиваем счетчик.
                            i++;
                            return encoded;

                        case 'emphasis':
                            return `${getTag('bpt', i, '*')}${traverseChildren(node, i + 1)}${getTag('ept', i, '*')}`;
